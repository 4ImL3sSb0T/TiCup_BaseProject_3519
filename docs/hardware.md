...
|------|---------------|------|------|--------------|
| 左电机 PWM | TIM_A0 CH0 | **B8** | DRV8701；主板有刷电机座 | 自改脚（原例程 A0） |
| 左电机 DIR | GPIO | **B10** | 主板有刷电机座 | 同上 |
| 右电机 PWM | TIM_A0 CH2 | **B12** | 主板有刷电机座 | E3_04_drv8701e_double |
| 右电机 DIR | GPIO | **B13** | 主板有刷电机座 | 同上 |
...
**当前左电机 DIR 已占用 B10**，方向模式右编码器不能直接照搬例程 B10，须另选脚。
...
| 轮 | PWM 宏 | 引脚 | DIR | 座子组 |
|----|--------|------|-----|--------|
| 左 `motor_left` | `PWM_TIM_A0_CH0_B8` | **B8** | **B10** | 跨 **M1/M4**（见下） |
...
| 组 | 脚对 | 本工程占用 |
|----|------|------------|
| **M1** | **B8 / B9** | **B8** = 左电机 PWM；**B9** = 右编码器 CH2（不可再当电机 DIR） |
| **M2** | **B12 / B13** | **B12** = 右电机 PWM，**B13** = 右电机 DIR（同组成对） |
| **M3** | **A27 / A26** | **A26/A27** = 左编码器正交（不可当电机） |
| **M4** | **B11 / B10** | **B10** = 左电机 DIR；**B11** 空闲（可扩第三路或将来左电机改回同组） |

| 主板电机座脚 | 本工程占用 | 说明 |
|--------------|------------|------|
| **B8** | **左电机 PWM** | M1；`PWM_TIM_A0_CH0_B8` |
| **B9** | 右编码器 CH2 | M1 另一脚；**不可给电机** |
| **B12** | **右电机 PWM** | M2；`PWM_TIM_A0_CH2_B12` |
| **B13** | **右电机 DIR** | M2 |
| **B10** | **左电机 DIR** | M4（因 M1 的 B9 给编码器，左 DIR 跨到 M4） |
| **B11** | 空闲 | M4 另一脚；方向编码器例程 G6 会占 B10/B11 |
| **A26 / A27** | 左编码器 A/B | M3 整组；**不可给电机** |

**注意：**

- 若将来改方向编码器（G6: B10/B11），须先把左电机 DIR 从 B10 挪开。  
- 若右编码器不再占用 B9，左电机宜改为 **M1 同组**（B8 PWM + B9 DIR）。
