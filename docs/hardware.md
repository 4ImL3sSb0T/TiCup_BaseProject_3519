...
| 轮 | PWM 宏 | 引脚 | DIR |
|----|--------|------|-----|
| 左 `motor_left` | `PWM_TIM_A0_CH0_B8` | **B8** | **B10** |
...
| 主板电机座脚 | 本工程占用 | 说明 |
|--------------|------------|------|
| **B8** | **左电机 PWM** | `PWM_TIM_A0_CH0_B8` |
| **B10** | **左电机 DIR** | GPIO |
| **B12** | **右电机 PWM** | `PWM_TIM_A0_CH2_B12` |
| **B13** | **右电机 DIR** | GPIO |
| **B11** | 空闲 | 可作扩展 DIR/PWM；若启用方向编码器例程 G6 则冲突 |
| B9 | 右编码器 CH2 | **不可给电机** |
| A26 / A27 | 左编码器 A/B | **不可给电机** |

**注意：** 若将来改方向编码器（G6: B10/B11），须先把左电机 DIR 从 B10 挪开。
