...
#ifndef __TINYEKF_H__
#define __TINYEKF_H__
...
#endif
#ifdef USE_CMSIS_DSP
// CMSIS-DSP 矩阵接口仅支持 float，若用户将 _float_t 改为 double 则直接报编译错误
typedef char tinyekf_requires_float[(sizeof(_float_t) == sizeof(float)) ? 1 : -1];
#endif
...
{
#ifdef USE_CMSIS_DSP
    arm_matrix_instance_f32 A = { (uint16_t)arows, (uint16_t)acols, (_float_t *)a };
    arm_matrix_instance_f32 B = { (uint16_t)acols, (uint16_t)bcols, (_float_t *)b };
    arm_matrix_instance_f32 C = { (uint16_t)arows, (uint16_t)bcols, (_float_t *)c };
    if (arm_mat_mult_f32(&A, &B, &C) == ARM_MATH_SUCCESS) {
        return;
    }
#endif
...
{
#ifdef USE_CMSIS_DSP
    arm_matrix_instance_f32 A = { (uint16_t)m, (uint16_t)n, (_float_t *)a };
    arm_matrix_instance_f32 AT = { (uint16_t)n, (uint16_t)m, (_float_t *)at };
    if (arm_mat_trans_f32(&A, &AT) == ARM_MATH_SUCCESS) {
        return;
    }
#endif
    for (int i=0; i<m; ++i)
...
#ifdef USE_CMSIS_DSP
    arm_matrix_instance_f32 A = { (uint16_t)m, (uint16_t)n, (_float_t *)a };
    arm_matrix_instance_f32 B = { (uint16_t)m, (uint16_t)n, (_float_t *)b };
    arm_matrix_instance_f32 C = { (uint16_t)m, (uint16_t)n, (_float_t *)c };
    if (arm_mat_add_f32(&A, &B, &C) == ARM_MATH_SUCCESS) {
        return;
    }
#endif
    for (int i=0; i<m; ++i) {
