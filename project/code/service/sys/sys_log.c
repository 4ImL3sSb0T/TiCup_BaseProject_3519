...
sys_log_type_e current_log_type = SYS_LOG_WIFI;

void sys_log_init(sys_log_type_e log_type) {
    current_log_type = log_type;
    if (current_log_type != SYS_LOG_WIFI && current_log_type != SYS_LOG_UART) {
        current_log_type = SYS_LOG_WIFI; // 默认使用 WIFI
    }

    // Initialize WIFI SPI module
    uint8_t try_count = 0;
